...
function TestimonialCard({ r }) {
  return (
    <div
      className="flex-shrink-0 rounded-2xl p-4 sm:p-5 flex flex-col justify-between"
      style={{
        width: "clamp(260px, 80vw, 320px)",
        minHeight: "160px",
        background:
          "linear-gradient(135deg, rgba(10,18,55,0.9) 0%, rgba(4,8,28,0.95) 100%)",
        border: "1px solid rgba(255,255,255,0.07)",
        boxShadow: "0 4px 24px rgba(0,0,0,0.35)",
      }}
    >
      <p className="text-gray-300 text-xs sm:text-sm leading-relaxed mb-4 sm:mb-5">
        "{r.text}"
      </p>
      <div className="flex items-center gap-2 sm:gap-3">
        <div
          className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
        ...
        <div className="flex-1 min-w-0">
          ...
          {r.role && (
            <div className="text-blue-400 text-[10px] sm:text-xs mt-0.5 truncate">
              {r.role}
            </div>
          )}
        </div>
...
// ── TrustedBy.jsx (Responsive) ──
// export default function TrustedBy() — kept at bottom to export separately
