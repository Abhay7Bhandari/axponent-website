...
function useGlowCanvas(cardRef, canvasRef) {
  useEffect(() => {
    ...
    const resize = () => { canvas.width = card.offsetWidth; canvas.height = card.offsetHeight }
    resize()
    ...
    const draw = () => {
      ...
      if (glow.opacity > 0) {
        ...
        let side = min === dT ? 'top' : min === dB ? 'bottom' : min === dL ? 'left' : 'right'
        ...
        let x1, y1, x2, y2
        if (side === 'top')    { x1 = Math.max(0,(p-half)*w); y1=0;  x2=Math.min(w,(p+half)*w); y2=0 }
        if (side === 'bottom') { x1 = Math.max(0,(p-half)*w); y1=h;  x2=Math.min(w,(p+half)*w); y2=h }
        if (side === 'left')   { x1=0;  y1=Math.max(0,(p-half)*h); x2=0;  y2=Math.min(h,(p+half)*h) }
        if (side === 'right')  { x1=w;  y1=Math.max(0,(p-half)*h); x2=w;  y2=Math.min(h,(p+half)*h) }
...
function StatCard({ num, label, className = '', style = {}, children, align = 'left' }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at center, rgba(0,90,200,0.07) 0%, transparent 70%)', zIndex: 1 }} />

      <div
        className="relative flex flex-col justify-center py-9 px-8"
        style={{ zIndex: 3, alignItems: align === 'center' ? 'center' : 'flex-start', textAlign: align }}
      >
        {children || (
          <>
            <div className="font-display font-bold leading-none mb-2"
              style={{ fontSize: 'clamp(2rem,3.8vw,3.25rem)', color: '#ffffff', letterSpacing: '-0.02em' }}>
              {num}
            </div>
      ...
      <section className="py-8 px-4">
        <div className="max-w-5xl mx-auto">
          <div style={{
            ...
            {/* Row 1 — 65+ | 3 | 4 */}
            <StatCard num={STATS[0].num} label={STATS[0].label} />
            <StatCard num={STATS[1].num} label={STATS[1].label} />
            <StatCard num={STATS[2].num} label={STATS[2].label} />

            {/* Row 2 — 120+ | 5Mn+ hero (spans 2) */}
            <StatCard num={STATS[3].num} label={STATS[3].label} />
            <StatCard
              num={STATS[4].num}
              label={STATS[4].label}
              align="center"
